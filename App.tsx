...
import { parseCSV, calculateTeamPerformance, getCancellationReasonSummary } from './dataProcessor';
import { LeadData, TeamPerformance } from './types';

const STATUS_COLORS = {
  Base: '#003049',      // Deep Navy
  Onboarding: '#0077b6', // Ocean Blue
  Cancelado: '#d62828',  // Alert Red
  Others: '#94a3b8'
};

// Expanded CSV representing the leads context
...
"RBF SISTEMAS DE SEGURANÇA CONTRA INCÊNDIO","BASE GE","4","Ganho","2025-03-25 08:31:19","Técnicos","Segurança, Serviços","","","2025-02-19 11:56:00"
...
"Don Luiz (Cancelado)","CANCELADO GE","4","Ganho","2025-07-30 18:21:27","Executivos","Bebidas","Dificuldade de contato com a diretoria por gestão interna.","Inadimplência (Jurídico)","2025-06-25 14:30:27"
...
"VETEX – Unidade Teresina","BASE GE","4","Ganho","2025-10-23 09:34:01","Entregadores","Animal/Veterinária","","","2025-09-09 11:08:03"
...
"Unatech","ETAPA 5: > 60 DIAS","5","Ganho","2025-09-26 18:04:12","Técnicos","Tecnologia","Dificuldade severa de contato com o Luiz.","","2025-09-11 20:57:49"
...
"RBL Distribuidora","BASE GE","13","Ganho","2025-10-13 08:34:08","Promotores","Animal/Veterinária","","","2025-09-18 11:52:00"
"Bluem brasil","BASE GE","8","Ganho","2025-10-01 08:51:03","Vendedores","Saúde","","","2025-09-18 14:46:52"
"GuaruSolar (Cancelado)","CANCELADO GE","4","Ganho","2025-09-29 16:17:22","Técnicos","Energia","","Mudança de Estratégia da Empresa","2025-09-18 17:39:36"
...
"FGR Telecom (Cancelado)","CANCELADO GE","4","Ganho","2025-10-09 10:54:46","Vendedores","Telecom","Redução de custos e sem equipe técnica.","Redução de Custos","2025-09-23 12:08:08"
"Rodonorte Transportes","BASE GE","5","Ganho","2025-10-29 13:59:17","Vendedores","Agronegócio","","","2025-09-24 11:19:22"
"WJ Comércio e Serviços LTDA","ETAPA 5: > 60 DIAS","4","Ganho","2025-09-26 17:56:27","Técnicos","Manutenção","Gestor muito ausente devido a viagens de negócio.","","2025-09-24 14:59:23"
...
"DrogaVET Nova Friburgo","ETAPA 1: 15 DIAS","4","Ganho","2025-12-01 08:55:37","Vendedores","Animal/Veterinária","","","2025-10-21 09:59:10"
...
"VETEX (DROGAVET)","BASE GE","10","Ganho","2025-10-27 16:09:48","Vendedores","Animal/Veterinária","","","2025-10-27 12:07:07"
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const processed = parseCSV(RAW_CSV);
    setData(processed);
    setLoading(false);
  }, []);
  ...
  const stats = useMemo(() => {
    ...
    const cancelado = data.filter(l => l.categoria === 'Cancelado').length;
    
    const closedLeads = data.filter(l => l.categoria === 'Base');
    const avgCloseTime = closedLeads.length > 0 
      ? Math.round(closedLeads.reduce((acc, curr) => acc + curr.diasParaFechar, 0) / closedLeads.length) 
      : 0;
    ...
    return { 
      ...
      onboarding, 
      cancelado, 
      convRate: total > 0 ? (base / total) * 100 : 0, 
      avgCloseTime 
    };
  }, [data]);

  const teamPerformance = useMemo(() => calculateTeamPerformance(data), [data]);
  const cancellationSummary = useMemo(() => getCancellationReasonSummary(data), [data]);

  const funnelData = useMemo(() => [
    { name: 'Total de Ganhos', value: stats.total, fill: '#002855' },
    { name: 'Em Onboarding', value: stats.onboarding, fill: STATUS_COLORS.Onboarding },
    { name: 'Base GE (Sucesso)', value: stats.base, fill: STATUS_COLORS.Base },
    { name: 'Cancelados', value: stats.cancelado, fill: STATUS_COLORS.Cancelado },
  ], [stats]);

  const insights = useMemo(() => {
    const topTeam = [...teamPerformance].sort((a,b) => b.taxaConversao - a.taxaConversao)[0];
    
    return [
      {
        title: "Eficiência de Conversão",
        text: `Atualmente ${stats.convRate.toFixed(1)}% dos leads da base se tornaram Base GE.`,
        icon: "fa-chart-line",
        tip: "Ação: Focar em leads parados para aumentar esse índice em 10%."
      },
      {
        title: "Campeão de Conversão",
        text: `A equipe de ${topTeam?.equipe || 'Vendedores'} lidera com ${topTeam?.taxaConversao.toFixed(1)}% de aproveitamento.`,
        icon: "fa-trophy",
        tip: "Ação: Replicar o modelo de vendas consultivas deste time para as demais frentes."
      },
      {
        title: "Perda Financeira",
        text: `O motivo "${cancellationSummary[0]?.reason || 'Inadimplência'}" é o maior driver de churn.`,
        icon: "fa-triangle-exclamation",
        tip: "Ação: Implementar alerta de inadimplência precoce no 15º dia de contrato."
      }
    ];
  }, [teamPerformance, cancellationSummary, stats.convRate]);

  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-900 text-white gap-6">
      <div className="w-16 h-16 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
      <span className="font-black tracking-[0.5em] uppercase text-sm">Processando Base de Leads...</span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 text-slate-900 font-sans selection:bg-blue-100">
      {/* Executive Header */}
      <header className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-end border-b-2 border-slate-200 pb-12 gap-8">
        <div className="space-y-3">
          <div className="flex items-center gap-4">
            <div className="bg-blue-800 text-white p-4 rounded-3xl shadow-xl shadow-blue-200">
              <i className="fa-solid fa-gauge-high text-3xl"></i>
            </div>
            <h1 className="text-5xl font-black text-blue-950 tracking-tighter uppercase italic">Analytics 2025</h1>
          </div>
          <p className="text-slate-500 font-bold ml-20 text-lg uppercase tracking-tight">Performance Operacional & Eficiência de Times</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full lg:w-auto">
          {[
            { label: 'Leads na Base', val: stats.total, color: 'text-blue-900' },
            { label: 'Conversão Real', val: `${stats.convRate.toFixed(1)}%`, color: 'text-emerald-700' },
            { label: 'Tempo Médio', val: `${stats.avgCloseTime}d`, color: 'text-cyan-700' },
            { label: 'Aguardando Onboarding', val: stats.onboarding, color: 'text-amber-600' }
          ].map((c, i) => (
            <div key={i} className="bg-white p-6 rounded-[2.5rem] border-2 border-slate-100 shadow-sm hover:shadow-xl transition-all group">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 group-hover:text-blue-500 transition-colors">{c.label}</span>
      ...
      <main className="max-w-7xl mx-auto py-12 space-y-12">
        {/* Main Section: Funnel & Action Plan */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Funnel Chart */}
          <div className="lg:col-span-8 bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-200/50">
            <h2 className="text-2xl font-black text-blue-950 mb-12 flex items-center gap-4 uppercase tracking-tighter">
              <span className="w-3 h-10 bg-blue-800 rounded-full"></span> Saúde do Funil de Onboarding
            </h2>
            <div className="h-[450px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={funnelData} layout="vertical" margin={{ left: 20, right: 120 }}>
                  <XAxis type="number" hide />
                  <YAxis dataKey="name" type="category" stroke="#94a3b8" fontSize={12} width={130} axisLine={false} tickLine={false} />
                  <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '24px', border: 'none', boxShadow: '0 20px 25px'}} />
                  <Bar dataKey="value" radius={[0, 20, 20, 0]} barSize={60}>
                    {funnelData.map((e, i) => <Cell key={i} fill={e.fill} />)}
                    <LabelList dataKey="value" position="right" offset={25} style={{ fill: '#002855', fontWeight: 900, fontSize: '1.8rem' }} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <h2 className="text-2xl font-black text-blue-950 mb-8 flex items-center gap-4 uppercase tracking-tighter">
              <span className="w-3 h-10 bg-amber-500 rounded-full"></span> Diagnóstico Gerencial
            </h2>
            {insights.map((insight, idx) => (
              <div key={idx} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 group hover:border-blue-300 transition-all">
                <div className="flex gap-6">
                  <div className="bg-slate-50 p-4 rounded-2xl h-fit group-hover:bg-blue-50 transition-colors">
                    <i className={`fa-solid ${insight.icon} text-blue-800 text-xl`}></i>
                  </div>
                  <div className="space-y-2">
                    <h3 className="font-black text-blue-950 text-xs uppercase tracking-widest">{insight.title}</h3>
                    <p className="text-slate-500 text-xs leading-relaxed">{insight.text}</p>
                    <div className="pt-3">
                      <div className="bg-blue-50 px-4 py-2 rounded-xl border border-blue-100">
                        <p className="text-blue-800 text-[10px] font-black italic uppercase tracking-tighter">
                          <i className="fa-solid fa-lightbulb mr-2"></i> {insight.tip}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        {/* Churn Reasons */}
        <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-200/50">
          <h2 className="text-2xl font-black text-red-800 mb-10 flex items-center gap-4 uppercase tracking-tighter">
            <span className="w-3 h-10 bg-red-600 rounded-full"></span> Motivos do Churn
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cancellationSummary.map((c, i) => (
              <div key={i} className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-red-200 transition-all">
                <span className="text-xs font-black text-slate-800 uppercase truncate pr-4">{c.reason}</span>
                <div className="flex items-center gap-3">
                  <div className="h-2 w-12 bg-red-100 rounded-full overflow-hidden">
                    <div className="h-full bg-red-600" style={{ width: `${(c.count / stats.cancelado) * 100}%` }}></div>
                  </div>
                  <span className="text-xs font-black text-red-600">{c.count}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-[3.5rem] shadow-sm border border-slate-200/50 overflow-hidden">
          <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-white">
            <h2 className="text-2xl font-black text-blue-950 uppercase tracking-tighter italic">Ranking de Performance Operacional</h2>
            <div className="bg-blue-900 text-white px-6 py-2 rounded-full shadow-lg shadow-blue-100">
              <span className="text-[10px] font-black uppercase tracking-[0.2em]">Visão Consolidada de Times</span>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 border-b border-slate-100">
                <tr>
                  <th className="px-12 py-8 text-[11px] font-black text-slate-400 uppercase tracking-widest">Equipe Principal</th>
                  <th className="px-6 py-8 text-[11px] font-black text-blue-950 uppercase text-center">Total</th>
                  <th className="px-6 py-8 text-[11px] font-black text-blue-800 uppercase text-center">Base</th>
                  <th className="px-6 py-8 text-[11px] font-black text-blue-400 uppercase text-center">Onb.</th>
                  <th className="px-6 py-8 text-[11px] font-black text-red-500 uppercase text-center">Canc.</th>
                  <th className="px-6 py-8 text-[11px] font-black text-slate-500 uppercase">Conversão Base</th>
                  <th className="px-6 py-8 text-[11px] font-black text-slate-500 uppercase">Cancelamento (%)</th>
                  <th className="px-12 py-8 text-[11px] font-black text-slate-500 uppercase">Tempo de Ciclo</th>
                </tr>
              </thead>
                  ...
                  <tr key={idx} className="hover:bg-blue-50/20 transition-all group">
                    <td className="px-12 py-8">
                      <div className="flex items-center gap-5">
                        <span className="w-10 h-10 flex items-center justify-center bg-blue-50 text-blue-900 text-sm font-black rounded-2xl border border-blue-100 group-hover:bg-blue-900 group-hover:text-white transition-all">{idx + 1}</span>
                        <span className="font-black text-slate-800 uppercase text-xs tracking-tight">{team.equipe}</span>
                      </div>
                    </td>
                    <td className="px-6 py-8 text-center font-bold text-slate-500 text-base">{team.total}</td>
                    <td className="px-6 py-8 text-center font-black text-blue-950 text-base">{team.base}</td>
                    <td className="px-6 py-8 text-center font-black text-blue-500 text-base">{team.onboarding}</td>
                    <td className="px-6 py-8 text-center font-black text-red-400 text-base">{team.cancelado}</td>
                    <td className="px-6 py-8 min-w-[200px]">
                      <div className="flex items-center gap-4">
                        <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                          <div className="h-full bg-blue-800 shadow-xl transition-all duration-1000" style={{ width: `${team.taxaConversao}%` }}></div>
                        </div>
                        <span className="text-xs font-black text-blue-950 w-12">{team.taxaConversao.toFixed(1)}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-8 min-w-[200px]">
                      <div className="flex items-center gap-4">
                        <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200">
                          <div className="h-full bg-red-600 shadow-xl transition-all duration-1000" style={{ width: `${team.taxaChurn}%` }}></div>
                        </div>
                        <span className="text-xs font-black text-red-600 w-12">{team.taxaChurn.toFixed(1)}%</span>
                      </div>
                    </td>
                    <td className="px-12 py-8">
                      <div className="flex items-center gap-3">
                         <div className="bg-cyan-50 p-2 rounded-lg"><i className="fa-solid fa-clock-rotate-left text-cyan-600 text-sm"></i></div>
                         <div className="flex flex-col">
                            <span className="text-sm font-black text-cyan-900 tracking-tighter">{team.mediaDiasFechamento} dias</span>
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Média Ciclo Ganho</span>
                         </div>
                      </div>
                    </td>
                  </tr>
      ...
      <footer className="max-w-7xl mx-auto text-center py-24 border-t-2 border-slate-200">
        <div className="w-40 h-2 bg-blue-900 mx-auto mb-10 rounded-full opacity-10"></div>
        <p className="text-slate-300 text-[11px] font-black uppercase tracking-[0.8em]">Analytics 2025 | Relatório Gerencial Consolidado | v4.5</p>
      </footer>
    </div>
  );
};
